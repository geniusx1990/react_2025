...
test('submits valid data and writes to store', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/^password$/i), { target: { value: 'Aa1!' } });
    fireEvent.change(screen.getByLabelText(/confirm password/i), { target: { value: 'Aa1!' } });
    fireEvent.click(screen.getByRole('radio', { name: /^male$/i }));
    fireEvent.click(screen.getByLabelText(/accept terms/i));
    fireEvent.change(screen.getByLabelText(/^country$/i), { target: { value: 'Lithuania' } });
