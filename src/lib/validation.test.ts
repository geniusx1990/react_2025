...
test('requires uppercase first letter in name', async () => {
    await expect(formSchema.validate({ ...base, name: 'john' }))
        .rejects.toBeTruthy();
});

test('disallows negative age', async () => {
    await expect(formSchema.validate({ ...base, age: -1 }))
        .rejects.toBeTruthy();
});
...
test('calculates flags correctly', () => {
    expect(getPasswordStrengthFlags('Aa1!')).toEqual({
        number: true, upper: true, lower: true, special: true,
    });
    expect(getPasswordStrengthFlags('aaaa')).toEqual({
        number: false, upper: false, lower: true, special: false,
    });
});
